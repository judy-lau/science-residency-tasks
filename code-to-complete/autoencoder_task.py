...
model. You may ask, what is an autoencoder? Well, first, no worries that you dont
know! We will help you to go through it! First, autoencoder is a neural network
that takes as input a datapoint X and at the output tries to reproduce that same X exactly
as it was at the input. There is one trick however, if your X is dimension 784,
autoencoder will attempt to compress that X into a feature space far smaller than that, say
32 using encoder neural network, and then reconstructing from this 32 dimensional feature
back into X using a decoder neural network.

In the code below you will have to use PyTorch.
See Pytorch documentation here https://pytorch.org/docs/stable/index.html.
See Pytorch tutorial here https://pytorch.org/tutorials/
